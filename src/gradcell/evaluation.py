...
def hard_cutoff_metrics(
    ...
    results = {}
    cutoffs = {}
    for rate_name, rate in (("1c", 1.0), ("5c", 5.0), ("6c", 6.0)):
        inputs = base.copy()
    ...
    result_1c, result_5c, result_6c = results["1c"], results["5c"], results["6c"]
    status = (
    ...
    return {
        ...
        "energy_retention_5c": result_5c.delivered_energy_wh / energy_1c,
        "energy_retention_6c": result_6c.delivered_energy_wh / energy_1c,
        "capacity_1c_ah": result_1c.delivered_capacity_ah,
        "capacity_5c_ah": result_5c.delivered_capacity_ah,
        "capacity_6c_ah": result_6c.delivered_capacity_ah,
...
def scalarized_loss(
    ...
    with torch.no_grad():
        values = objective(
            ...
            torch.from_numpy(np.asarray(retention_5c)).double(),
            torch.from_numpy(np.asarray(retention_6c)).double(),
