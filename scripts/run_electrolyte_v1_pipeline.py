...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the physics-only electrolyte DFN pipeline.")
    ...
    if min(args.physics_samples, args.physics_validation_samples, args.physics_test_samples) <= 0:
        parser.error("all physics partition sample counts must be positive")
    ...
    run(
        [
            ...
            str(args.physics_validation_samples),
            "--physics-test-samples",
            str(args.physics_test_samples),
            "--seed",
            str(args.seed),
            "--output",
            str(data_path),
    ...
    run(
        [python, "scripts/validate_electrolyte_v1_gradients.py", "--backend", args.physics_backend],
        root,
    )
    run(
        [
